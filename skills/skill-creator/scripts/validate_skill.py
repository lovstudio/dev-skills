...
def validate_workbuddy_source(root: Path, errors: list[str]) -> None:
    validate_source(root, errors, require_self_contained=True)
    workbuddy = root / "workbuddy"
    validate_connector_meta(workbuddy / "connector-meta.json", errors)
    icons = [
        workbuddy / "icon.svg",
        workbuddy / "icon.png",
        workbuddy / "icon.jpg",
    ...
    if not any(path.is_file() for path in icons):
        errors.append(f"{workbuddy}: add icon.svg, icon.png, or icon.jpg")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--target",
        choices=("source", "workbuddy", "workbuddy-package"),
        default="source",
...
def main() -> int:
    ...
    elif args.target == "workbuddy":
        validate_workbuddy_source(root, errors)
    else:
